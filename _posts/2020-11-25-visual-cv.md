...
generated, so read along.

<!--more-->

## Motivation

Recently I give a presentation with an overview of my career, with a
focus on data analysis / bioinformatics. As my path to data analysis has
not been straightforward, I wanted walk the audience through my career
as we went along the several stages. Because I am someone who generally
takes pride on my presentations, and this was about me after all, and I
wanted something:
...
presented it had to be done 100% with code. No post-production in
Inkscape, and in keeping with this mantra, the slide-deck itself was
done with `LaTeX`.

Because the plot is so intrinsically linked with the presentation
itself, I will also explain my though process on certain choices and how
that is linked with the presentation[1].

## Framework

Important for me, due to to way my career as paned out so far,and the
goal of the presentation was to [2]:

-   depict my journey from wet lab to data analysis, with increasing
    amounts of time spent analysis data.
...
Initially I thought of using a Gantt chart as those are the got to plot
for timelines and there are several packages for this [3]. However,
after prototyping with pen and paper, I decided it would be too hard to
incorporate all the features I wanted to have without cluttering the
plot. Basically, it didn’t feel right.

Luckily inspiration was in one of my favorite
...
with the richness of information (also I am terrible at
drawing/painting).
...
font in out R plots. There a couple of steps that need to be done before
...
You can also also the third snippet of code make sure the desired font
is indeed available in `R`.
...
The data science trajectory line is a smooth line result of linear model
fitted to the time points in the CV. The model was chosen based on the
very accurate statistical measure that the fit needed be visually
appealing - smooth and in an upward trajectory after a particular time
point.

To add the small icons, I ended up `magick` which I could only install
with conda (well, [mamba](https://github.com/mamba-org/mamba)), due to
missing dependencies. I did try the R way with
`install.packages("magick")` but gave up after the second error. Life is
too short.
...
them so much, but I will credit the author, [Tippawan
Sookruay](https://thenounproject.com/wanny4/), nevertheless.
...
few option in terms of packages (patchwork for example), but in the end
`magick` was the best one for my purposes.

But we are not done yet: career milestones and cities I lived are still
missing.
...
that my name follows Portuguese naming conventions, and each publishers
seems to ignore what I write in the provided forms and just makes up a
different version of my name. Not helping matters is that my first and
last name are fairly common, so there were a lot of false positives in
the pubmed queries. In ORCID some of the variables I needed, namely the
publication date, were missing for some papers.

I do keep all my publications in a `bibtex` file exported from zotero,
so that in end was the route I took. The publication data was extracted
with the help of `bib2df` to read in the bibtex file, and `rcrossref` to
retrieve publications dates which were incomplete or missing in the
bibtex file.
...
analysis, of the y-axis. So now I also need to estimate the Y positions
in this plot.

## ggplot hacking

I have done enough custom plots, and spent enough hours googling on how
to do it, that I know that `ggplot` objects store a ton of information
which can be accessed and modified (at you own risk). So what I did was
to extract the `x` and `y` coordinates of the fitted line from the
`ggplot` object.
...
Now I have the timeline coordinates, but these need to be matched with
the dates of the publication. The x-axis values (dates) of the fitted
plot are intervals which do not match the CV dates. To solve that
problem, as nearly any data manipulation problem, `data.table` and
rolling joins came to the rescue. The goal was to the closest time point
of my CV dates in the fitted plot.
...
And a similar join was done for some key outreach dates that I also want
...
I could have used point with different color/shape to highlight my
academic degrees, but these belong to different category of events than
publications or other more technical milestones / achievements, and it
wold add too much information to the lines anyway. As an alternative I
decided to use arrows and text to point out when those degrees where
obtained.

Again there is nothing magic or novel about this, in fact I took
inspiration from [Cedric
Scherer](https://cedricscherer.netlify.app/top/dataviz/), except that
need for trial and error to get the positions correct. The positions of
...
the plots with the final dimensions to gauge that. Saving as pdf gave me
the best results vs saving the plots as png. There is a commented out
`ggsave` at the end which was used to save the plots with everything in
it’s right place.
...
-   milestones (papers) are also depicted as round points, and in
    keeping with my trajectory there some in which I contributed only
    lab work, others with data analysis.
-   the milestones with the diamond shape are to be described in the
    presentation (blog, R package maintainer).

I hope you can appreciate that this plot contains a lot of information,
but doesn’t feel cluttered, that is, a decent amount of blank space.
Also, this is not meant to be a self-contained, self-explanatory piece
of data visualization - I will be guiding the audience through the
pieces of information and revealing them only when needed.

## Getting there

Well, nearly final. There are still a couple of modifications to be done
for the presentation:

1.  No legend because I wanted to talk about the milestones during the
    presentation without giving way too early what the points meant;
2.  Indication of the places of work is still missing.

The latter is crucial to the presentation because I will be following
each plot with an highlighted place of work with the my role there and
task there. So in effect highlighting the plots in a stepwise manner is
critical for having breaks and setting the tone for each part of the
presentation.

Removing the legend fairly simple (once you know who to do it and I
always google it).
...
And that was it. It me quite some time to get it done, including testing
packages, but in the end I was quite happy the result. It was a lot of
work, but I wanted to impress and I did.
...
software, etc, which I am not sure it’s always taken into account. Not
that it affects me because I am not looking to climb the academic
ladder.

[4] The slide-deck is not available, but if a few people show interest I
could make put it online.
